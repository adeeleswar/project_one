...
NES_SERVER_CONF=$(pwd)/nes.cfg
export NES_SERVER_CONF
CMDLINE_CONF_PATH="$PWD/nes.cfg"
export CMDLINE_CONF_PATH
...
lcov --capture --directory ${BUILD_PATH} --output-file ${COVERAGE_INFO} \
    --rc lcov_branch_coverage=1 > /dev/null 2>&1
lcov --extract ${COVERAGE_INFO} "*/daemon/*" -o ${COVERAGE_INFO} \
    --rc lcov_branch_coverage=1 > /dev/null 2>&1
lcov --remove ${COVERAGE_INFO} "*/rnis/*" --remove ${COVERAGE_INFO} "*tests/daemon/*" \
    -o ${COVERAGE_INFO} --rc lcov_branch_coverage=1 >> summary.txt 2>&1
genhtml ${COVERAGE_INFO} --output-directory --rc lcov_branch_coverage=1 out > /dev/null 2>&1
